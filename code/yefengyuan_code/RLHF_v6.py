...
for i in range(len(reward)):
    x = reward[i] // 10
    y = reward[i] % 10
    map[x][y] = random.randint(100, 500)
    # map[x][y] = 1000

for i in range(len(block)):
    x = block[i] // 10
    y = block[i] % 10
    map[x][y] = random.randint(-5000, -1000)
    # map[x][y] = 1000
...
def q_learning(map_data, start, end, gamma=0.8, alpha=0.1, epsilon=0.1, max_iterations=100000):
    rows, cols = map_data.shape
    ...
    while start != end and t < max_iterations:
        ...
        if 0 <= new_row < rows and 0 <= new_col < cols:
            ...
            reward = map_data[new_row][new_col]  # Assign a reward based on the map_data
...
path_value = []
for i in range(500):
    path = q_learning(map, start_point, end_point)
    q_learning_path.append(path)
    v = compute_reward(path, 0.95)
    path_value.append(v)
    print(v)

index_max = path_value.index(max(path_value))
q_learning_path_max = q_learning_path[index_max]

    
print("Best Route (Q-learning):", q_learning_path_max)
print("Best Route value (Q-learning):", path_value[index_max])

# print(q_learning_path)
x2 = np.array([coord[1] for coord in q_learning_path_max])  # 从路径中提取 x 坐标
y2 = np.array([coord[0] for coord in q_learning_path_max])  # 从路径中提取 y 坐标
